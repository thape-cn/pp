...
function GradeSquare({ row, column, calibration_template, children, onDrop, onCountingGradePeople, group_level }) {
  ...
  const [{ isOver }, drop] = useDrop(() => ({
    accept: 'personCard',
    drop: item => {
      if ((item.group_level == "manager_a" && item.person_row == row) || item.group_level == "staff" || group_level == "auxiliary" || group_level == "supervisor" || group_level == "manager_b") {
        onDrop(item, row, column);
      }
    },
    collect: monitor => {
      const item = monitor.getItem();
      const isAllowed = item && ((item.group_level == "manager_a" && item.person_row == row) || item.group_level == "staff" || group_level == "auxiliary" || group_level == "supervisor" || group_level == "manager_b");
      return {
