...
class FinalizeCalibrationSessionMailerTest < ActionMailer::TestCase
  test "notification_manager" do
    mail = FinalizeCalibrationSessionMailer.with(manager: User.find_by(email: "guochunzhong@thape.com.cn"), calibration_session: CalibrationSession.first).notification_manager
