...
- Always call `authorize` and use `policy_scope`
...
- Include `stimulus` and `datatables` packs on relevant pages
- CoreUI for UI components
- Mount React components with specific DOM IDs
- Append packs using `append_javascript_pack_tag` helper

### DataTables
- Server-side processing with AjaxDatatablesRails
- Define `view_columns`, `data`, and `get_raw_records` in Ruby
- Pass column configuration via data attributes
...
- Mailer tests in `test/mailers/`
- Use `bin/rails test:db` to reset database before testing

## Key Configuration Files

- `config/shakapacker.yml` - Shakapacker configuration
- `config/webpack/webpack.config.js` - Webpack customization
- `.rubocop.yml` - StandardRB configuration
- `config/initializers/omniauth.rb` - OpenID Connect settings
- `bin/dev` - Development server startup script
- `bin/setup` - Project initialization

## Important Patterns

1. **Always include "stimulus"** - With webpack 5, loading sequence matters
2. **Use namespaced controllers** - Inherit role-based access control
3. **Pundit authorization** - Authorize every action, scope collections
4. **Server-side DataTables** - Handle large datasets efficiently
5. **React for complex UIs** - Use for scoring interfaces and calibration panels
6. **Sidekiq for background work** - Email, notifications, imports, cleanup
